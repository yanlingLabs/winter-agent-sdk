...
describe("--offline is what CI runs, and it is inert", () => {
  ...
  test("an EDITED overlay survives a real regeneration byte-for-byte, and the edit reaches the output", () => {
    // HAZARD, stated because it is real: this test MUTATES a repository file and restores it in a
    // `finally`. A SIGKILL between the write and the restore leaves the probe string in
    // `overlay/models.json` — recoverable with `git checkout`, but a surprise. It is not run against
    // a mkdtemp copy because `scripts/provider-catalog.ts` is FROZEN and resolves its paths from its
    // own `import.meta.url`, so it accepts no root argument to point elsewhere; a copy of the overlay
    // could not be regenerated by the real writer, and a re-implementation of the writer is exactly
    // the second serializer this lane refuses to have. The restore is from saved BYTES rather than by
    // regenerating, so every ordinary failure path is covered.
    //
