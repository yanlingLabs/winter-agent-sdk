...
//
// P6 FIX WAVE (Ruling E-2): RETIRED FROM PRODUCTION -- A TEST HELPER, NOTHING MORE. The bookkeeping
// below (the pending slot, the trigger-to-reason mapping, the "owner cancels" contract) duplicated
// state the engine already owns (`pendingModelSwitch`, `interruptCurrentTurn`, the quiescent
// boundary), and the whole-branch review found the duplicate never wired while the engine's own
// policy corrupted the continuity record. The engine's `applyPendingModelSwitch` now calls the two
// PURE functions directly -- `classifySwitch` and `buildPortableHandoff` -- through the runtime's
// `resolveModelSwitch` seam. This object is exported from the continuity sub-barrel ONLY (never the
// package barrel), for `winter-provider-conformance`'s §12.3 corpus cases that script a switch
// against it; no production code may reach it by package name, and none does.
//
