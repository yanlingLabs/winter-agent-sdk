...
function buildRequestBody(req: TurnRequest, descriptor: WinterModelDescriptor | undefined, opts: GoogleAdapterOptions, ctx: ProviderContext, purpose: "generate" | "count" = "generate"): Record<string, unknown> {
  ...
  // reads that telemetry as a size. Never content, only the fact and how many times.
  for (let i = 0; i < droppedForeignReasoning; i++) {
  ...
  return {
    ...
    async headers(ctx, policy, json) {
      ...
      const headers: Record<string, string> = {
        // WS-13b HONEST IDENTITY. FIRST, so a user connection profile can still override it.
