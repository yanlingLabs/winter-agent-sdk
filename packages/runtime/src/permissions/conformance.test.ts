...
const WS08_12: ConformanceRow[] = [
  ...
  },
  {
    ...
    status: "deferred",
    owningPhase:
      "P5, per RULING P2-N (P2 fix-wave item 12): Notification is RECLASSIFIED not-P2-fireable -- phase ruling 3's own P2-fireable list was mislabeled in naming it; notification emission is a HOST-SURFACE concern (a settings-hot-reload notice, an auto-mode-fallback-state UI notice, or similar), which has no Winter-internal trigger condition to fire from at P2 and belongs with P5's settings/filesystem-hooks work, the same owning phase hook_progress (Carry 2, WS08-PROGRESS row below) already carries for the analogous reason. Evidence this ruling rests on (unchanged from before the ruling): engine.ts has ZERO call sites for Notification -- its only 7 fireObservationalHook(...) call sites are SessionStart/UserPromptSubmit/PermissionDenied/PostToolUse/PostToolUseFailure/Stop/SessionEnd (PreToolUse/PermissionRequest fire from evaluator.ts separately) -- and WS-08 itself only says 'runtime notifications' fire it, without naming a concrete Winter-internal trigger condition.",
    note:
      "Ruling P2-N resolves what the pre-ruling text left as an open question for the controller: phase ruling 3's list is amended in effect (Notification moves to the typed-but-inert set, owned by P5) rather than a P2 firing site being added retroactively. This task's file list does not include engine.ts, so no firing site is added here regardless.",
  },
  {
    id: "WS08-02",
