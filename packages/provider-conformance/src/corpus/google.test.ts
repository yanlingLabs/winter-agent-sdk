...
describe("Google GenerateContent: the live request", () => {
  ...
  test("`hostHeaders`: a CREDENTIAL name is dropped on BOTH endpoint kinds; a non-credential identity name only on a user one (F-3 / M-4)", async () => {
    ...
    // THE FIXTURE THE RULING NAMES, on both endpoint kinds: a profile carrying a credential-shaped
    // header and a tracing one — only the tracing one rides. Before F-3 the whole map rode a
    // generated endpoint, and on a user endpoint only the four IDENTITY names were dropped, so a
    // host `cookie` reached every Anthropic, Google and Vertex request ever sent.
    for (const policy of [generated.policy, user.policy]) {
      expect(hostHeaders(policy, { cookie: "session=abc", "x-trace": "keep" })).toEqual({ "x-trace": "keep" });
      expect(hostHeaders(policy, { authorization: "Bearer nope", "proxy-authorization": "Basic nope", "x-api-key": "nope", "api-key": "nope", "x-goog-api-key": "nope", "x-amz-security-token": "nope", "x-trace": "keep" })).toEqual({
        "x-trace": "keep",
      });
    }

    const supplied = { "x-goog-user-project": "p", "X-GOOG-USER-PROJECT": "p2", "openai-organization": "o", "x-goog-quota-project": "q", "x-trace": "keep" };
    // A reviewed descriptor endpoint still vouches for an identity header that belongs to it — but
    // only for one that is NOT credential-shaped. `x-goog-quota-project` is the single name where
    // the two lists differ; the other three privileged names are on `CREDENTIAL_HEADER_NAMES` too
    // and are therefore dropped even here.
    expect(hostHeaders(generated.policy, supplied)).toEqual({ "x-goog-quota-project": "q", "x-trace": "keep" });
    ...
    for (const name of ["cookie", "proxy-authorization", "x-api-key", "api-key", "x-goog-api-key", "x-amz-security-token", "authorization"]) {
      expect([name, CREDENTIAL_HEADER_NAMES.includes(name)]).toEqual([name, true]);
    }
  });
...
describe("Google GenerateContent: the pure mapping", () => {
  ...
  test("a CREDENTIAL-shaped host header never rides: `cookie` + `x-trace` -> only `x-trace` (F-3 / M-4)", async () => {
    // `hostHeaders` stripped identity names only, so a host `cookie` or `proxy-authorization` in a
    // connection profile reached this family's wire on every request. A profile is non-secret
    // connection metadata by contract (WS-13 §6): a credential in it is a misconfiguration to drop,
    // never a second auth channel to honour.
    const adapter = testGoogleAdapter();
    await withFake({ routes: googleCorpusRoutes() }, async (fake) => {
      await foldTurn(
        adapter,
        { model: GOOGLE_MODELS.main, messages: [{ role: "user", content: "a" }] },
        googleContext(fake.url, {
          connection: { providerId: "google", baseUrl: fake.url, local: true, headers: { cookie: "session=SMUGGLED-COOKIE", "proxy-authorization": "Basic SMUGGLED-PROXY", "x-trace": "keep" } },
        }),
      );
      const recorded = fake.requests[0]!;
      ...
      expect(noRequestContains(fake, "SMUGGLED-PROXY")).toBe(true);
    });
  });
