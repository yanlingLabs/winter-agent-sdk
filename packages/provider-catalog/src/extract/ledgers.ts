...
export const FIELD_PROVENANCE: readonly FieldProvenance[] = [
  ...
  { field: "model.upstreamId / displayName / aliases", provenance: "copied-verbatim", note: "upstream `id` / `name` / `aliases`; a duplicate id is dropped and recorded rather than silently de-duplicated" },
  ...
  { field: "model.contextWindow / maxInputTokens / maxOutputTokens", provenance: "copied-verbatim", note: "upstream `contextLength` (falling back to the provider's `defaultContextLength`), `maxInputTokens`, `maxOutputTokens`. Non-positive or non-integer values are dropped" },
  { field: "model.inputModalities / outputModalities", provenance: "mechanically-normalized", note: "`text` plus `image`/`audio`/`video` from upstream's `supportsVision`/`supportsAudio`/`supportsVideo`; output is `text` (upstream states no output modality)" },
  { field: "model.toolCalling / nativeTools", provenance: "mechanically-normalized", note: "upstream `toolCalling` mapped to `native`/`none`. ABSENT upstream becomes `none` at `confidence: \"unknown\"` — fail-closed, because `native` is what makes a model agent-eligible (WS-13 §8.1)" },
  ...
  { field: "model.status", provenance: "local-override", note: "every extracted row is `candidate`. Promotion to `supported` requires the behavioural corpus (WS-13 §13); upstream presence never promotes anything" },
