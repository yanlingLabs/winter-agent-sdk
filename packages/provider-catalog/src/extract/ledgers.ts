...
export const FIELD_PROVENANCE: readonly FieldProvenance[] = [
  ...
  { field: "provider.defaultEndpoints", provenance: "mechanically-normalized", note: "`api` is upstream's `baseUrl` with EXACTLY the path its own `format` names removed (`/chat/completions`, `/responses`, `/v1/messages`, `/v1beta/models`) — the API ROOT, and the exact inverse of what every adapter in that family appends to `connection.baseUrl`. Recording the verbatim path instead is a silent misroute once a row is consumed rather than shadowed (measured: `.../chat/completions/chat/completions`), and the five hand-authored overlay endpoints are byte-identical to what this strip produces. Winter still never trims to an ORIGIN, and a `default`-executor row whose URL does not end in its format's suffix FAILS the run rather than shipping with no `api` (an absent one inherits the adapter's own vendor default). `responsesBaseUrl`/`modelsUrl` are copied verbatim; the native-cloud executors (`bedrock`, `vertex`) are exempt — single-provider adapters whose `api` is never copied into a connection. A URL carrying userinfo or a query string is dropped and recorded (R6-11)" },
  ...
  { field: "reasoning.continuationDomain / summaryRequest / readableState / completionEvent / toolLoopRequirement / effortRequest / blockBinding / perMessageEffort", provenance: "official-doc-derived", note: "OVERLAY ONLY. Continuation domain is a first-class documented fact, never inferred from a shared HTTP shape (WS-13 §8.2). `perMessageEffort` names its mechanism: Anthropic's `{beta}` or OpenAI's `{item: \"configuration_update\"}`" },
