...
const WIDENING_ROWS: ConformanceRow[] = [
  ...
  },
  {
    ...
    status: "new",
    citations: [
      { file: `${SCRIPTS}/verify-provider-live.test.ts`, testName: "`anthropic` (Lane S round 3): with the `ant` broker wired via `overrides`, runs against a real `ant` stub and prints the `anthropic:console` CREDENTIAL_REF -- NEVER `anthropic:default`, and NO claude executable is built or supplied" },
  ...
  },
  {
    ...
    spec: WS13B,
    bullet:
      "§3/D20 Anthropic Console OAuth, RETIRED-AND-REPLACED 2026-09-13 (P10a-1 + same-day amendment; corrected Lane S round 2; account fixed Lane S round 3): the derived PKCE login against the CONSOLE host (Claude Code's own private client id, re-implemented) was deleted after the platform refused its grant for every derivable request shape; Console OAuth now runs ONLY through Anthropic's own `ant` broker binary (`auth login`/`auth logout`/`auth print-credentials`), spawned by this SDK (never a re-implementation of the OAuth protocol, and never `claude` -- measured to write no Anthropic profile for this org), writing the ORDINARY `bearer` credential kind under its OWN fixed account, `anthropic:console` -- NEVER `anthropic:default`, the account the user's pasted api-key lives under (R6-10; an Opus review caught the original account choice as a live data-loss defect, since fixed); the OAuth beta stays a PROTOCOL header on a turn authenticated with an `oauth`-kind credential, and API-key auth stays the default",
    status: "new",
    citations: [
      { file: `${PROVIDER_RUNTIME}/adapters/anthropic/console-broker.test.ts`, testName: "REDACTION: every line reaching `onLine` has its URL's query string stripped -- the one-time code never appears in any line" },
      { file: `${PROVIDER_RUNTIME}/adapters/anthropic/console-broker.test.ts`, testName: "SUBMITCODE + DONE (success): the right code completes the login, writes the profile, and mints the bearer material" },
      { file: `${PROVIDER_RUNTIME}/adapters/anthropic/console-broker.test.ts`, testName: "refreshAnthropicBearer: the ant binary's own failure is a named, typed refusal — never the bare exit code alone, and any existing material is left untouched" },
      { file: `${PROVIDER_RUNTIME}/adapters/anthropic/console-oauth.test.ts`, testName: "`CONSOLE_BEARER.betaHeader` is the derived value, not typed from memory -- the ONE field of the old CONSOLE_OAUTH table still shipped" },
      { file: `${CORPUS}/anthropic.test.ts`, testName: "oauth material still rides as a Bearer with the OAuth beta as a PROTOCOL header -- RETIRED: no self-refresh is attempted, near or far from expiry alike" },
      ...
      { file: `${RUNTIME}/provider/credential-api.test.ts`, testName: "`anthropic` (Lane S round 3): with the `ant` broker fully wired, this door spawns the real `ant` stub, awaits `readConsoleCode`, and answers with the `anthropic:console` ref -- NEVER `anthropic:default`, and NO claude executable is built or supplied" },
      { file: `${PROVIDER_RUNTIME}/adapters/anthropic/console-broker.test.ts`, testName: "DATA-LOSS REGRESSION (Opus review, Lane S round 3): a full login -> refresh -> logout cycle never touches api-key material stored at anthropic:default" },
      { file: `${CORPUS}/anthropic.test.ts`, testName: "SECURITY (Opus review, Lane S round 3, data-loss fix): a `bearer` credential found under `anthropic:default` -- the api-key slot -- is a TYPED REFUSAL, never dispatched and never treated as the api-key credential" },
    ],
    note: "The credential is the ORDINARY keychain ref, not a new `CredentialRef` kind — which is why the live gate's `keychain:<account>` locator addresses it with no adaptation (review round 1, minor 8, confirmed against the merged code, and unchanged by the P10a-1 retirement since the ref shape never depended on which login wrote it). The ACCOUNT is `anthropic:console`, not `anthropic:default` (Lane S round 3, an Opus-review data-loss fix): the console bearer and the user's pasted api-key are now two separate records, and the last two citations pin that a `bearer` stranded at the api-key's account is refused rather than dispatched.",
  },
  {
    id: "WS13b-8",
    spec: WS13B,
...
const MODEL_FAMILIES_ROWS: ConformanceRow[] = [
  ...
  },
  {
    ...
    citations: [
      ...
      { file: `${PROVIDER_RUNTIME}/adapters/anthropic/console-oauth.test.ts`, testName: "`anthropicCredentialRef` is the ONE spelling of the record name — a host never assembles it by hand" },
      { file: `${RUNTIME}/provider/credential-api.test.ts`, testName: "`anthropic` (Lane S round 3): with the `ant` broker fully wired, this door spawns the real `ant` stub, awaits `readConsoleCode`, and answers with the `anthropic:console` ref -- NEVER `anthropic:default`, and NO claude executable is built or supplied" },
    ],
    note: "\"Resolves only to anthropic\" is §4 step 3-i's VENDOR-LEADS rule, not an exclusion of every other provider: the shipped catalog carries other rows for `claude-opus-5` too (the second citation's own `rows.length` assertion is `toBeGreaterThan(1)`), and they serve the slot when `anthropic` is disabled or unpreferred — a different, legal path §4 already covers (WS13c-5). What D28 actually excludes is a CREDENTIAL KIND the resolver never even sees: `hasCredential` is presence-only, and the third/fourth citations pin that Winter's own credential surface for `anthropic` can only ever be populated by an api-key (its own record, `anthropic:default`) or by Winter's OWN Console OAuth flow (host-brokered per P10a-1, through `anthropic:console`'s own ONE record spelling — a SEPARATE account from the api-key's, Lane S round 3) — never by a claude.ai (consumer subscription) credential, for which this repository has no acquisition path and no storable shape under any provider id. No test asks the resolver for a claude.ai-flavoured credential BY NAME, because there is no such `CredentialMaterial` to construct; these four, spanning `runtime` and `provider-runtime`, are the closest real proof and the ones the plan's own self-review pointed at.",
  },
];
