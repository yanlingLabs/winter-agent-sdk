...
const WIDENING_ROWS: ConformanceRow[] = [
  ...
  },
  {
    ...
    status: "new",
    citations: [
      { file: `${SCRIPTS}/verify-provider-live.test.ts`, testName: "`anthropic` (Lane S round 2): with the `ant` broker wired via `overrides`, runs against a real `ant` stub and prints the `anthropic:default` CREDENTIAL_REF -- NO claude executable is built or supplied" },
  ...
  },
  {
    ...
    spec: WS13B,
    bullet:
      "§3/D20 Anthropic Console OAuth, RETIRED-AND-REPLACED 2026-09-13 (P10a-1 + same-day amendment; corrected Lane S round 2): the derived PKCE login against the CONSOLE host (Claude Code's own private client id, re-implemented) was deleted after the platform refused its grant for every derivable request shape; Console OAuth now runs ONLY through Anthropic's own `ant` broker binary (`auth login`/`auth logout`/`auth print-credentials`), spawned by this SDK (never a re-implementation of the OAuth protocol, and never `claude` -- measured to write no Anthropic profile for this org), writing the ORDINARY `bearer` credential kind under `anthropic:default` (R6-10); the OAuth beta stays a PROTOCOL header on a turn authenticated with an `oauth`-kind credential, and API-key auth stays the default",
    status: "new",
    citations: [
      ...
      { file: `${RUNTIME}/provider/credential-api.test.ts`, testName: "`anthropic` (Lane S round 2): with the `ant` broker fully wired, this door spawns the real `ant` stub, awaits `readConsoleCode`, and answers with the `anthropic:default` ref -- NO claude executable is built or supplied" },
    ],
    note: "The credential is the ORDINARY keychain ref (`anthropic:default`), not a new `CredentialRef` kind — which is why the live gate's `keychain:<account>` locator addresses it with no adaptation (review round 1, minor 8, confirmed against the merged code, and unchanged by the P10a-1 retirement since the ref shape never depended on which login wrote it).",
  },
  {
    id: "WS13b-8",
    spec: WS13B,
