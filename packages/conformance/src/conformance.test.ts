...
const WIDENING_ROWS: ConformanceRow[] = [
  ...
  },
  {
    ...
    status: "new",
    citations: [
      { file: `${SCRIPTS}/verify-provider-live.test.ts`, testName: "`anthropic` (P10a-1 amendment): with the broker wired via `overrides`, runs against real `claude`/`ant` stubs and prints the `anthropic:default` CREDENTIAL_REF" },
  ...
  },
  {
    ...
    spec: WS13B,
    bullet:
      "§3/D20 Anthropic Console OAuth, RETIRED-AND-REPLACED 2026-09-13 (P10a-1 + same-day amendment): the derived PKCE login against the CONSOLE host (Claude Code's own private client id, re-implemented) was deleted after the platform refused its grant for every derivable request shape; Console OAuth now runs ONLY through Anthropic's own broker binaries (`claude auth login --console` / `ant auth print-credentials`), spawned by this SDK (never a re-implementation of the OAuth protocol), writing the ORDINARY `bearer` credential kind under `anthropic:default` (R6-10); the OAuth beta stays a PROTOCOL header on a turn authenticated with an `oauth`-kind credential, and API-key auth stays the default",
    status: "new",
    citations: [
      ...
      { file: `${RUNTIME}/provider/credential-api.test.ts`, testName: "`anthropic` (P10a-1 amendment): with the broker fully wired, this door spawns the real `claude`/`ant` stubs, awaits `readConsoleCode`, and answers with the `anthropic:default` ref" },
    ],
